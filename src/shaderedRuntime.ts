...
export interface ISHADERedBreakpoint {
	id: number;
	line: number;
	verified: boolean;
}
...
/**
 * A SHADERed runtime with minimal debugger functionality.
 */
export class SHADERedRuntime extends EventEmitter {
	...
	// maps from sourceFile to array of SHADERed breakpoints
	private _breakPoints = new Map<string, ISHADERedBreakpoint[]>();
	...
	/**
	 * "Step into" for SHADERed debug means: go to next character
	 */
	...
	/**
	 * "Step out" for SHADERed debug means: go to previous character
	 */
	public stepOut() {
		if (typeof this._currentColumn === 'number') {
			this._currentColumn -= 1;
	 ...
	 */
	public async setBreakPoint(path: string, line: number): Promise<ISHADERedBreakpoint> {

		const bp: ISHADERedBreakpoint = { verified: false, line, id: this._breakpointId++ };
		let bps = this._breakPoints.get(path);
		if (!bps) {
			bps = new Array<ISHADERedBreakpoint>();
	 ...
	 */
	public clearBreakPoint(path: string, line: number): ISHADERedBreakpoint | undefined {
